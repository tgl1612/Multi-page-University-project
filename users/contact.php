...
$seller_data = $_GET['seller'];


//get user info query//
$read = "SELECT * FROM elmtree_users WHERE elmtree_users.user_id = '$seller_data' ";
...
//query to get user rating average//

$readavgrating = "SELECT ROUND( AVG(user_rating), 0)
FROM elmtree_ratings
WHERE user_id ='$seller_data'";

$resultavgrating = $conn->query($readavgrating);
...
<div class= 'container-fluid mycategoryitems'>
  ...
  if(!$result){
        ...
        while($row = $resultavgrating->fetch_assoc()){
          $avgrating = $row['ROUND( AVG(user_rating), 0)'];
        }
         ...
         <div class = 'col-xs-6 col-sm-6 col-md-6 largeimage'>
          ...
          <h4>User rating: $avgrating/10</h4>

          <div class ='userinfo'>

          <a href='message.php?sender=$seller_data'><button type='button' class='btn btn-primary contact'>Message</button></a>
          <a href = 'userrating.php?ratingid=$seller_data'><button type = 'button' class ='btn btn-primary contact'>Rate</button></a>
