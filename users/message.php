...
$senderdata = mysqli_real_escape_string($conn, $_GET['sender']);
$readmessages = "SELECT * FROM `elmtree_message`
                 WHERE user_to_id = '$user_id' AND user_from_id = '$senderdata'
                 OR user_from_id = '$user_id' AND user_to_id = '$senderdata'
                 ORDER BY timesent ASC";

$resultmessages = $conn->query($readmessages);
...
//get sender info
$getsenderID = "SELECT * FROM elmtree_users
                WHERE user_id = $senderdata";
$senderID = $conn->query($getsenderID);
      ...
      <div class= 'col-12 myiteminfo'>
        <div class='row'>
          ...
          <div class='col-12'>
            ...
            while($row = $senderID->fetch_assoc()){
              $sender = $row['user'];
            ...
            <h5 class ='regwelc1'>Chat with <a href = 'contact.php?seller=$senderdata'>$sender</a></h5>
            ";
...
echo"
          ...
          <div class = 'row mymessagesend'>




         <form action='messageprocess.php' method='POST' enctype='multipart/form-data' style ='width: 100%'>
         <input type='hidden' name ='sellerid' id='seller' value ='$senderdata'>
              <div class='form-group'>
                <input type='text' class='form-control' name ='postmessage' id='message' placeholder='Send message...'   class='validate' maxlength='500'>
              </div>


              <div class ='Myloginbut'>
              <button type='submit' name='submit' class='btn btn-primary loginbut'>Send</button>
            </div>
            </form>
          </div>
