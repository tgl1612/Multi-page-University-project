...
$user_id = $_SESSION['40023794_elmtree_userid'];
  ...
  $rating_id=$_GET['ratingid'];


//query to get seller info
  $readratinginfo = "SELECT * FROM elmtree_users WHERE user_id = '$rating_id'";

$resultrating = $conn->query($readratinginfo);
         ...
         <div class='row'>
           ...
           <div class='col-12'>
             ...
             while($row = $resultrating->fetch_assoc()){
               ...
               $seller_name = $row['user'];
               $seller_id = $row['user_id'];
             }

             echo"
             <h3 class ='regwelc1'>Rate user: $seller_name </h3>


             <form action='ratingprocess.php' method='POST' enctype='multipart/form-data'>
            <input type = 'hidden' value ='$seller_name' name= 'sellername'>
             <input type = 'hidden' value ='$seller_id' name= 'sellerid'>
             <div class='form-group'>
             <label for='Sellerref'>Unique Seller ID</label>
             <input type='number' name = 'sellerref' class='form-control' aria-describedby='ratingHelp' maxlength='10'>
             <small id='ratingHelp' class='form-text text-muted'>Ask a seller for thier unique seller ID after a purchase</small>
             </div>

             <div class='form-group'>
             <label for='rating'>Rating</label>
             <input type='number' name = 'rating' aria-describedby='ratingHelp2' class='form-control'  maxlength='2'>
             <small id='ratingHelp2' class='form-text text-muted'>Rate the seller from 1 (bad service) to 10 (great service)</small>
             </div>
                 ...
                 <button type='submit' name='submit' class='btn btn-primary loginbut'>Rate</button>
                 </div>
             </form>
             ";
              ?>
